...
import { pacientes, citas, getPaciente } from "@/lib/mock-data";
...
export default function PacientesPage() {
  ...
  const pacienteActivo = seleccionado ? getPaciente(seleccionado) : null;
  const citasPaciente  = seleccionado
    ? citas.filter(c => c.pacienteId === seleccionado).sort((a,b) => b.fecha.localeCompare(a.fecha))
    : [];

  const totalCobrado = citasPaciente
    .filter(c => c.estadoPago === "pagado")
    .reduce((s, c) => s + (pacienteActivo?.sesionPrecio ?? 0), 0);

  const totalPendiente = citasPaciente
    .filter(c => c.estadoPago !== "pagado")
    .reduce((s, c) => s + (pacienteActivo?.sesionPrecio ?? 0), 0);
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-5 gap-6">
        ...
        <div className="col-span-3">
          {!pacienteActivo ? (
            <div className="bg-white rounded-2xl border border-slate-100 h-full flex items-center justify-center">
              ...
              <div className="bg-white rounded-2xl border border-slate-100 p-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="flex items-center gap-2 text-sm text-slate-600">
                    ...
                    {citasPaciente.length} sesiones en total
                  </div>
                  <div className="flex items-center gap-2 text-sm text-slate-600">
                    <CreditCard className="w-4 h-4 text-slate-400" />
                    {pacienteActivo.sesionPrecio} € / sesión
                  </div>
                </div>
              </div>
                ...
                <div className="divide-y divide-slate-50">
                  {citasPaciente.map(cita => (
                    <div key={cita.id} className="flex items-center gap-4 px-5 py-3.5">
                      ...
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${estadoCitaColor[cita.estado]}`}>
                        {cita.estado}
                      </span>
